...
create table empoyees (id int8 generated by default as identity, box_id int8 not null, user_id int8 not null, primary key (id));
...
alter table if exists empoyees add constraint FKhi50wt4fqcn5pn04b7q1ltqpo foreign key (box_id) references boxes;
alter table if exists empoyees add constraint FKl0l2t3xcxfbxi28r945vto8us foreign key (user_id) references users;
...
insert into users(user_type, first_name, last_name, password, "role", surname, username) values
('user','Адамов', 'Даниил','password','ADMIN','Васильевич','admin'),
('employee','Работающий', 'Работяга','password','EMPLOYEE',null,'empl'),
('user','Заказывающий', 'Заказчик','password','ADMIN',null,'user');

insert into empoyees(box_id, user_id) values (1,2);
